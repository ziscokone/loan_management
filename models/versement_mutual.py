...
class MutualReversement(models.Model):
    _name = 'mutual.reversement'
    _description = 'Suivi versement des prêts mutuels'
...
# -------------------------------------METHODES ---------------------------------------------
    ...
    def _onchange_fichier_excel(self):
        ...
        try:
            ...
            # Recherche des demandes dont les chèques ont été delivrés
            demandes_validees = self.env['loan.mutual'].search([
                ('campaign_id', '=', self.campagne_id.id),
                '|',  # Opérateur OR
                ('state', '=', 'check_delivery'),
                ('state', '=', 'check_withdrawn')  # Deuxième statut ajouté
            ])
